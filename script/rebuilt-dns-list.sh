...
cat hosts*.txt | grep -v -E "^((#.*)|(\s*)|(\!.*))$" \
 ...
 | sed 's/0.0.0.0/127.0.0.1/g' | sed 's/::/127.0.0.1/g' | sed 's/127.0.0.1 //g' \
 |grep -Pv "((25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))" | sort \
...
| sed -e "s/127.0.0.1 //g" | sed 's/[ ]//g'\
| sort|uniq >pre-rules.txt

cat pre-rules.txt base-dead-hosts.txt base-dead-hosts.txt |sort \
 |uniq -u |grep -Pv "^((25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))$" > 1.txt
